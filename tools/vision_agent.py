...
def build_report_prompt(task: str, role: str, role_text: str, history: list[dict[str, Any]]) -> str:
    ...
    return f"""
请基于真实操作日志，输出一份接近“用户研究/产品体验走查”质量的中文体验报告。报告要像真人体验者写出来的，而不是简单测试日志。
重要：不要编造未执行步骤，不确定就写“不确定/需补测”。截图证据由程序在飞书文档的“逐步截图证据”章节自动上传，你在正文中必须用“见 Step 03 截图 / 见 Step 08 截图”引用对应证据。
...
报告要求：
1. 报告不能只写最终阻断点。即使任务未完整完成，也要覆盖已经真实体验过的每个阶段：主界面、添加源入口、源选择页、Game Capture 设置页、添加后画布效果、Go LIVE / Live info / 直播中 / 关播（如果到达）。
2. 先定义体验者人设和关注清单，再写任务概述、实际完成情况、理想标准对照、认知演变、分阶段过程分析、问题与建议汇总、总体评估。
3. 每个阶段都要包含：用户目标、我的预期、实际发生了什么、作为该角色的感受/情绪、阶段评分、问题、亮点。
4. 问题列表要结构化：序号、问题描述、严重程度（P0/P1/P2/P3）、所属阶段、证据截图 Step、用户影响、改进建议。
5. 至少输出 5 个候选体验问题；如果日志事实不足 5 个，说明哪些是“观察不足/需补测”，不要硬编。
6. 重点评价目标角色会不会理解：当前是否已准备好、画面是否被观众看到、默认值是否合理、专业术语是否友好、错误提示是否能指导下一步、按钮/入口是否明确、等待/加载是否有反馈。
7. 如果最终已经走完开播流程，体验可以结束；游戏源黑屏/捕获失败应作为问题写进报告，而不是作为继续反复修复的理由。
8. 正文中不要插入 Markdown 图片语法，也不要写本地图片路径作为图片；只写“见 Step xx 截图”。飞书图片由工具自动上传到附录。

报告结构：
标题：# TikTok LIVE Studio {role}体验报告

一、用户人设与关注清单
- 用户人设：用第一人称写出角色背景、技能水平、目标、担忧。
- 关注清单：用表格列出 5-8 个关注维度。
...
四、体验过程分析
- 按阶段写，例如：开播前设置、触发开播、直播中、关播与结束、数据回顾。
- 每个阶段必须包含“用户目标 / 我的预期 / 实际发生了什么 / 阶段简表评估 / 本阶段问题 / 本阶段亮点”。

五、问题与建议汇总
- 用表格列出所有问题，列：序号、问题描述、严重程度、所属阶段、证据截图 Step、用户影响、改进建议。
...
七、附录说明
- 写明“逐步截图证据由工具自动附在飞书文档后半部分，正文通过 Step 编号引用”。
"""
